...
**AVIF** zusätzlich zu WebP, immer aus dem PNG (`tools/bilder-avif.py`): am Karminrot-Poster 24 statt 45 KB bei kleinerer Abweichung vom Original. JavaScript entscheidet beim Bildwechsel über ein 1-px-AVIF. **Aufgeräumt:** `haus.js`, `werkbank.js` und die i18n-Zweige `beweis`, `haus`, `probieren` sind aus dem Repository raus. Auf dem Webspace liegen die beiden Skripte noch: Der Push mit der Abrissliste in `ftp-deploy.yml` wurde abgewiesen, weil das gh-Token auf dem Windows-Rechner kein `workflow`-Recht hat. Nachholen nach `gh auth refresh -s workflow` (oder im KAS löschen).
